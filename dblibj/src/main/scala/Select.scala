...
object Select {
  def getPreviousOperationResultSet(id: Int, state: OCDBState): Option[ResultSet] =
    lookUpConnList(id, state) match {
      case None => None
      case Some((_, pConn)) => pConn.result match {
  ...
  def ocesqlExecSelectIntoOne(id: Int, query: Option[String], nParams: Int, nResParams: Int, state: OCDBState): Unit = {
    ...
    if(fields != nResParams) {
      errorLogLn(s"ResParams(${nResParams}) and fields(${fields}) are different")
      setLibErrorStatus(OCDB_EMPTY(), state)
      return ()
    }
  ...
  def ocesqlExecSelectIntoOccurs(id: Int, query: Option[String], nParams: Int, nResParams: Int, state: OCDBState): Unit = {
    if(query.getOrElse("").length == 0 || state.globalState.occursInfo.iter > 500) {
      setLibErrorStatus(OCDB_EMPTY(), state)
      return ()
    }
    ...
    lookUpConnList(id, state) match {
      ...
      case Some((_, conn)) => conn.result match {
        case Right(EResultSet(rs)) => resultSetToSqlVarSelectOccurs(rs, state.globalState.sqlResVarQueue, state.globalState.occursInfo)
        case _ => ()
      }
    }
  }

  def resultSetToSqlVarSelectOccurs(rs: ResultSet, sqlVarQueue: Queue[SQLVar], occursInfo: OccursInfo): Int = {
    var rowCount = 0
  ...
  def resultSetToSqlVar(rs: ResultSet, index: Int, count: Int, sqlVarQueue: Queue[SQLVar], occursInfo: OccursInfo): Int = {
    sqlVarQueue(0).addr.getOrElse(nullDataStorage).memset(0, occursInfo.length * occursInfo.iter)
    var rowNum : Int = 0
    while(rs.next()){
      rowNum = rowNum + 1
      for((sv, j) <- sqlVarQueue.zipWithIndex) {
